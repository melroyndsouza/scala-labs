...
object CollectionExercise01 {
  ...
  def googleCodeJamGooglerese(lines: String*): Seq[String] = {
    ...
    def getMap(ip: String, op: String): Map[Char, Char] = {
      ip.toCharArray.filter(c => c != ' ').zip(op.toCharArray.filter(c => c != ' ')).toMap
    }

    val conversionMap = getMap("ejp mysljylc kd kxveddknmc re jsicpdrysi", "our language is impossible to understand") ++
      getMap("rbcpc ypc rtcsra dkh wyfrepkym veddknkmkrkcd", "there are twenty six factorial possibilities") ++
      getMap("de kr kd eoya kw aej tysr re ujdr lkgc jv", "so it is okay if you want to just give up") ++
      Map(' '->' ')

    lines.map(line => line.map(c => conversionMap.getOrElse(c,'?')))
  }
}
...
object CollectionExercise02 {
  ...
  def groupAdultsPerAgeGroup(persons: Seq[Person]): Map[Int, Seq[Person]] = {
    persons.filter(_.age > 18).groupBy(p => (p.age/10)*10).mapValues(_.sortBy(_.name))
  }
}
...
object CollectionExercise04 {
  ...
  def calcLengthLongestWord(lines: String*): Int = {
    lines.flatMap(line => line.split(" ")).maxBy(_.length).length
  }
}
...
object CollectionExercise05 {
  /**
    ...
    */
  def filterWithFoldLeft(seq: Seq[Int], modResult: Int = 0): Seq[Int] = {
    seq.foldLeft(List[Int]())((list, i) => if(i%2 == modResult) list:+i else list)
  }
  ...
  def groupByWithFoldLeft(seq: Seq[Int]): Map[Boolean, Seq[Int]] = {
    //val t = seq.foldLeft((List[Int](),List[Int]()))((lists, i) => if(i%2 == 0) (lists._1:+i,lists._2) else (lists._1,lists._2:+i))
    //Map(true -> t._1, false -> t._2)

    Map(true -> filterWithFoldLeft(seq,0), false -> filterWithFoldLeft(seq,1))
  }
}
